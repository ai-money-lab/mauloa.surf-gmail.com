...
async function generateImage(options: Options): Promise<string[]> {
  ...
  if (!apiKey && !useVertexAI) {
    console.error("\x1b[31mError:\x1b[0m GEMINI_API_KEY or Vertex AI service account key is required.");
    console.error("");
    ...
    console.error("  5. Vertex:   Place service account key at ~/.nano-banana/vertex-ai-key.json");
    console.error("");
  ...
  let ai: GoogleGenAI;
  if (useVertexAI) {
    // Read service account key to get project_id
  ...
  // imageConfig and tools are only supported on direct Gemini API, not Vertex AI
  if (!useVertexAI) {
