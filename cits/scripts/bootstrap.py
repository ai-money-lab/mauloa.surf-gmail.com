...
# 2170 config
ENTRY_PRICE = 580
MIN_PROFIT_PRICE = 585  # +5 yen margin
    ...
    try:
        ...
        if price:
            ...
            if price >= MIN_PROFIT_PRICE:
                result["decision"] = "SELL_OK"
            else:
                result["decision"] = "HOLD_NO_LOSS_RULE"

        log(f"  Price: {price} (prev={prev}, change={result.get('change_pct', '?')}%)")
        log(f"  PnL: {result.get('pnl_vs_entry', '?')} yen")
...
# ---------------------------------------------------------------------------
# Step 4: Start vps_agent in background
# ---------------------------------------------------------------------------
def start_vps_agent():
...
def main():
    ...
    try:
        ...
        log(f"  env_fix error: {e}")

    # Step 2: Analyze
    analysis = analyze_2170()
    result["analysis"] = analysis

    # Step 3: Sell if profit
