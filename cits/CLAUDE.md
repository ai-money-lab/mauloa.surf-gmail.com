...
## 絶対ルール（2026-04-13 追加 — 違反禁止）

### 背景・反省
2026-04-13月曜、本番稼働初日にVPS上のgit.exeが破損しており、
タスクスケジューラも未登録、vps_agentも停止した状態で「準備完了」と報告した。
ローカルコードのテストしか行っておらず、VPS上の実動作を一切確認していなかった。
これは重大な失敗。以下のルールを以後絶対に守る。

### 5つの絶対ルール

**① 確認の証拠としてVPSからの実行ログを必ず提示する**
- commands.jsonを投入するだけでは「確認」ではない
- vps_status.jsonに結果が返ってきて、その内容を引用して初めて「確認完了」
- ログなしに「動いています」「登録されています」と言うことは禁止

**② vps_status.jsonに結果が届くまで完了と言わない**
- VPS関連のあらゆる作業は vps_status.json の結果を確認するまで「未完了」
- 「投入しました」「送りました」は完了ではない。結果を見るまでが仕事
- vps_agentが応答しない場合は「未確認」として明示し、別手段を探す

**③ 第三者が見て再現できる手順書を作る**
- 「準備完了」と言う前に、知らない人が手順書だけ見て同じ状態を作れるか確認
- チェックリストは「やった」ではなく「実行結果のログ」で埋める
- 手順に抜けがあれば自分で気づいて補完する

**④ 何が確認できていないかを先に列挙する**
- 作業開始時に「確認済み／未確認」を明示する
- 「たぶん大丈夫」は未確認と同じ。未確認項目は必ず列挙して報告する
- 未確認のまま「完了」と言うことは禁止

**⑤ 本番前に必ずpaper modeで発注テストをする**
- コードが正しくても、VPS上で実際に動くとは限らない
- 本番発注の前に必ずpaper modeまたはdry-runで動作を確認する
- 発注テストの出力ログを提示して初めて「本番準備完了」と言える
...
❌ 確認ではないもの:
  - ローカルでのpytest/ruff通過
  - commands.jsonへのコマンド投入
  - 「動くはず」「登録されているはず」という推測
  - GitHubへのpush完了

✅ 確認とみなせるもの:
  - vps_status.jsonに実行結果が届き、内容を引用している
  - VPS上でのコマンド実行ログが手元にある
  - schtasks出力でCITSタスクが実際に列挙されている
  - kabuStation APIのトークン取得成功レスポンスを確認した
```

### VPS本番準備チェックリスト（全項目に実行ログが必要）

```
[ ] git.exe動作確認: VPS上で git --version の出力ログ
[ ] vps_agent稼働確認: vps_status.jsonにタイムスタンプ付き結果が届いた
[ ] タスク登録確認: schtasks /FO CSV出力に全9タスクが存在する
[ ] kabuStation起動確認: ポート18080でtoken取得成功のレスポンス
[ ] 発注テスト確認: paper mode / dry-runで発注ログが出力された
[ ] ポジション確認: get_positions()の実際のレスポンス
```

### AIが「完了」と報告してよい条件

上記チェックリストの全項目に、**VPSからの実際の出力ログ**が添付されている場合のみ。
1項目でも未確認なら「未完了」として列挙し、完了とは言わない。
...
- VPS git動作: ✅ [実行ログ]
- VPSタスク登録: ✅ [schtasks CSV出力]
- kabuStation API: ✅ [token取得レスポンス]
- 発注テスト: ✅ [dry-run出力ログ]
