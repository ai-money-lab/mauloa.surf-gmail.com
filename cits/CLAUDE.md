...
### ポジション状況（2026-04-10 JST）
- **2170 リンクアンドモチベーション**: 100株保有中。エントリー約580円
  - ❌ **ルール違反の失敗**: 620円利確指値を設定したが、ExpireDay=0（当日限り）で大引けで自動失効。**取れるはずだった+40円×100株=¥4,000の利益を逃した**
  - ❌ **再発防止**: kabu_api.pyのExpireDayを指値は7日後（5営業日）に修正済み。成行は当日
  - 現状: 603円前後。マイナス決済禁止ルールで580円以下では売らない（プラスに戻るまでホールド）
  - sell_2170.pyは価格チェック→MIN_PROFIT_PRICE(585円)以上のみ発注
- **白鳩(3192)**: 売却済み（+¥13,535）

### ルール違反の記録（忘れるな）
1. **2026-04-09**: 2170の620円利確指値が`ExpireDay=0`で自動失効。設定ミスで取れる利益を逃した
   - 原因: kabu_api.pyの`"ExpireDay": 0`ハードコード
   - 修正: 指値は7日後まで有効に変更
   - 教訓: **指値注文は必ず複数日有効にする。当日限りで利確指値を出してはいけない**
...
### 絶対遵守ルール
1. **絶対にマイナスでは決済しない。** `current_price <= entry_price` なら絶対に売らない。ストップ割れでも、チャート悪化でも、時間切れでも、**マイナス決済は禁止**。ホールドし続けてプラスに戻るのを待つ
2. 損をするような発注は絶対禁止。購入根拠を明確にしてから発注
3. **毎回の発言の冒頭で `TZ=Asia/Tokyo date` を実行し、JST時刻を表示せよ。** セッション切替・時間経過を見逃すな。「さっき08:30だった」は通用しない。今が何時かを毎回確認しろ
4. 「問題ない」と嘘をつくな。実行結果のみが事実
5. 自立しろ。毎回HIROKIに確認するな
6. PLAN B/C必須。CIS式だけに依存しない
7. 取引した日は mauloa.surf@gmail.com にレポートをメール送信
