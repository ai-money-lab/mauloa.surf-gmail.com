...
def compute_exit_score(
    ...
    """Compute chart-based exit score for an open position.

    Args:
        opens/highs/lows/closes/volumes: Price history (most recent last).
        entry_price: Original entry price.
        current_high: Highest price since entry.
        hold_days: Number of days held.
        strategy: "CIS" or "KEI" (affects weight profile).

    Returns:
        ExitScore with total score and component breakdown.
    """
    if len(closes) < 5:
    ...
    # Override: trailing stop hit = always EXIT
    if trail_hit:
        total = max(total, 75.0)

    # Override: stage breakdown = strong EXIT signal
    if stage_breakdown:
...
def _score_bearish_patterns(
    ...
    """Score bearish reversal patterns in last 5 bars."""
    if len(closes) < 3:
        return 0.0
...
def _score_momentum_decay(closes: list[float], volumes: list[float]) -> float:
    """Score momentum exhaustion signals."""
    if len(closes) < 10:
        return 0.0
