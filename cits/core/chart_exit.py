...
def compute_exit_score(
    ...
    if len(highs) >= 15 and len(lows) >= 15:
        ...
        if atr > 0 and current_high > 0:
            if profit_pct >= 10: trail_pct = 3.0
            elif profit_pct >= 5: trail_pct = 2.0
            elif profit_pct >= 3: trail_pct = 0.0
            else: trail_pct = (2.0 * atr / current_high) * 100
    ...
    if trail_hit: total = max(total, 75.0)
    if stage_breakdown: total = max(total, 60.0)
    if total >= 70 or trail_hit: rec = "EXIT_NOW"
    elif total >= 50: rec = "EXIT"
    elif total >= 30: rec = "WATCH"
    else: rec = "HOLD"
...
def _score_bearish_patterns(opens, highs, lows, closes):
    ...
    for i in range(-n + 1, 0):
        ...
        if prev_body > 0 and body < 0 and c < prev_o and o > prev_c: score += 35
        if h > 0 and (h - low) > 0:
            us = h - max(o, c)
            ls = min(o, c) - low
            rb = abs(body)
            if us > 2 * rb and ls < rb * 0.3: score += 25
        if prev_body > 0 and body < 0 and o > prev_c and c < (prev_o + prev_c) / 2: score += 30
    if n >= 3 and all(closes[i] < opens[i] for i in range(-3, 0)): score += 20
...
def _score_momentum_decay(closes, volumes):
    ...
    current = closes[-1]
    ma5 = sum(closes[-5:]) / 5
    if current < ma5: score += min((ma5 - current) / ma5 * 100 * 15, 40)
    if len(closes) >= 10:
        ma10 = sum(closes[-10:]) / 10
        if current < ma10: score += min((ma10 - current) / ma10 * 100 * 10, 30)
    if len(volumes) >= 10:
        rv = sum(volumes[-3:]) / 3
        pv = sum(volumes[-10:-3]) / 7 if sum(volumes[-10:-3]) > 0 else 1
        if pv > 0 and rv / pv < 0.5: score += min((1 - rv / pv) * 40, 30)
    if len(closes) >= 5 and closes[-1] < max(closes[-5:-1]): score += 10
    rsi = _compute_rsi_simple(closes)
    if rsi is not None and rsi > 70: score += min((rsi - 70) * 2, 20)
    return min(score, 100.0)
