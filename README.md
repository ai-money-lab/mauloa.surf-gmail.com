...
HIROKIの専門性と経験を5つの柱に分類し、投稿比率をコントロールする。

| # | 柱名 | 投稿比率 | テーマ例 |
|---|------|---------|---------|
| 1 | **不動産の裏側** | 30% | 仲介手数料の仕組み、おとり物件の実態、事故物件の告知義務、特殊清掃の現場、施工コーディネーターという仕事 |
| 2 | **住環境・暮らしの知恵** | 25% | 間取りの読み方、リフォームで価値が上がる工事、退去費用トラブル、防音・結露対策、防犯カメラの選び方 |
| 3 | **お金・資産・不動産投資** | 20% | 利回りの罠、空室リスクの計算、築古vs新築、エリア選定の鉄則、ローンの組み方 |
| 4 | **経営者の日常** | 15% | 平日サーフィンのルーティン、愛犬KOAとの暮らし、コーデックス栽培、ジム立ち上げ、46歳の体づくり |
| 5 | **テクノロジー×不動産** | 10% | MATTERPORT活用、VR内見、AI×不動産の未来、不動産DX、AI防犯カメラ |
...
`hook_power` / `persona_match` / `pillar_alignment` / `algorithm_optimization` / `no_external_links` / `no_banned_content` / `character_limit` / `number_included` / `cta_ending` / `originality` / `tone_balance` / `algorithm_hooks`
